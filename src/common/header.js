...
import CustomizedBadges from '../ui/cart';
import { useSelector } from "react-redux"
const Header = ({ setcategory, category }) => {
    const store = useSelector(data => data)
    const [search, setSearch] = useState('');
    const handleSearchVaule = () => {
        setcategory(search);
        setSearch('')
    }
    return (
        <div className="flex justify-between w-full">
            <div className="flex gap-x-4 items-center  w-2/4 ">
                <Link to="/home">
                ...
                {/* <div className=" md:flex md:gap-x-6">
                    <a className="inline-block rounded-lg py-1 px-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900" href="/#features">
                        Features</a>
                    <a className="inline-block rounded-lg py-1 px-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900" href="/#testimonials">Testimonials</a>
                ...
                <div class="input-group flex ml-5 w-3/4">
                    <select className=" font-normal bg-[#F0F2F2] w-24 text-black  rounded-lg rounded-r-none px-2 py-1 form-select" aria-label="Default select example">
                        <option selected className="font-semibold ">ALL</option>
                        <option value="1">Men's Collections</option>
                        <option value="2">Women's Collections</option>
                        <option value="3">Electronics </option>
                        <option value="4">Toys</option>
                        <option value="5">Kids</option>
                    </select>
                    <input id="email" type="text" class="form-control cursor-pointer" name="email" value={search} onChange={(e) => {
                        setSearch(e.target.value)
                        setcategory(search);
                    }} placeholder="Search..." />
                    <span class="input-group-addon " style={{ paddingRight: "20px" }} onClick={handleSearchVaule}>  <i class="fa fa-search"></i></span>
                </div>

            </div>
            <div className="flex gap-x-3 items-center ">
                <span className="text-2xl hidden md:flex">Sign Up</span>
                <button className="px-6 py-3  text-xl  rounded-full bg-blue-600 text-white hover:text-slate-100 hover:bg-blue-500">Login</button>
                <Link to="/wishlist" > <FavoriteIcon className='text-red-600 !text-4xl hover:text-red-500' /></Link>
                <Link to="/cart"   > <CustomizedBadges length={store && store.cart.length} /></Link>
            </div>
        </div>
